...
w0 = pd.read_csv('w0_'+k+'times.csv',encoding= 'utf-8',header=None)
w1 = pd.read_csv('w1_'+k+'times.csv',encoding= 'utf-8',header=None)
w2 = pd.read_csv('w2_'+k+'times.csv',encoding= 'utf-8',header=None)
...
for times in range(train_times):
    ...
    '''
    w2_ = np.dot(act1.T,-0.2*loss_out*prediction*(1-prediction))
    w1_ = np.dot(act0.T,-0.2*loss_1*act1*(1-act1))
    w0_ = np.dot(X_train.T,-0.2*loss_0*act0*(1-act0))
    ...
    if((times+1)%1000 ==0):
        with open('w0_'+str(times+10001)+'times.csv','w',newline="") as t:
            writer=csv.writer(t)
            writer.writerows(w0)
        
        with open('w1_'+str(times+10001)+'times.csv','w',newline="") as n:
            writer=csv.writer(n)
            writer.writerows(w1)

        with open('w2_'+str(times+10001)+'times.csv','w',newline="") as m:
            writer=csv.writer(m)
            writer.writerows(w2)
